...
same draft, same build. We keep decks whose player is at or above --min-winrate.
  ...
  python extract_decks.py --set FDN --format PremierDraft --min-winrate 0.60

Data: https://www.17lands.com/public_datasets (downloaded and cached under data/).
Outputs (under out/<set>_<format>_wr<min>/):
...
e.g. Foundations plus SpecialGuests for FDN's bonus-sheet cards.
  ...
  python extract_decks.py --set FDN --dck --xmage-jar .../lib/mage-sets-1.4.58.jar \
...
def fetch(set_code: str, fmt: str) -> str:
    path = os.path.join(HERE, "data", f"game_data_public.{set_code}.{fmt}.csv.gz")
...
def find_javap() -> str:
    for cand in (os.path.join(os.environ.get("JAVA_HOME", ""), "bin", "javap"), shutil.which("javap"),
                 os.path.join(HERE, "..", "jdk21", "Contents", "Home", "bin", "javap")):
...
def xmage_card_numbers(jar: str, set_classes: list[str]) -> dict[str, tuple[str, str]]:
    ...
    cache = os.path.join(HERE, "data", "xmage_" + "_".join(set_classes) + ".json")
    if os.path.exists(cache) and os.path.getmtime(cache) >= os.path.getmtime(jar):
        return {k: tuple(v) for k, v in json.load(open(cache)).items()}
    javap = find_javap()
    out: dict[str, tuple[str, str]] = {}
    tmp = os.path.join(HERE, "data", "_classes")
...
def main():
    ...
    ap.add_argument("--xmage-jar", default=os.path.join(HERE, "..", "pr-dense-vocab", "xmage-latest", "lib",
                                                        "mage-sets-1.4.58.jar"))
    ...
    out = args.out or os.path.join(HERE, "out", f"{args.set}_{args.format}_wr{int(args.min_winrate * 100)}")
